...
impl<T> Page<T> {
    ...
    pub fn page_query(total_row:u64, extend_param: &ExtendPageDTO) -> Page<T> {
        ...
        let mut result = Page::<T> {
            ...
            total_row,
            total_page: 0,
            page_no:if extend_param.page_no.is_some() { extend_param.page_no.unwrap() } else { 1 },
            page_size:if extend_param.page_size.is_some() { extend_param.page_size.unwrap() } else { 10 },
        };
        //计算总页数
