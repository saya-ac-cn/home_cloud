...
pub async fn execute_mysqldump_body() {
    let start_time =
        DateTimeUtil::naive_date_time_to_str(&Some(DateUtils::now()), &util::FORMAT_Y_M_D_H_M_S);
    // 确定要备份的路径
...
pub async fn undone_plan_notice_body() {
    ...
    if query_list_result.is_err() {
        // 查询此刻的计划提醒发生异常
        error!(
            "触发定时任务后，查询未完成的计划提醒发生异常:{}",
            query_list_result.unwrap_err()
        );
        return;
    }
...
pub async fn do_plan_notice() {
    ...
    let query_where = PlanPageDTO {
        id: None,
        standard_time: Some(date.clone()),
        cycle: None,
        unit: None,
        title: None,
        content: None,
    ...
    for mut plan in plans {
        ...
        if next_exec_time_op.is_none() {
            error!(
                "无效的日期(standard_time={},cycle={},unit={})循环周期",
                &standard_time.clone(),
                plan.cycle.unwrap(),
                plan.unit.unwrap()
            );
    ...
    pub async fn init_system_scheduler() {
        ...
        if plan_result.is_ok() {
            ...
            for plan in plans {
                ...
                scheduler.add_task(
                    build_plan_notice_async_task(plan.id.unwrap(), cron_tab.as_str()).unwrap(),
                );
            }
        }
