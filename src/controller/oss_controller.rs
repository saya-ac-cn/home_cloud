...
pub async fn upload_base64_picture(
    ...
    let vo = CONTEXT
        .oss_service
        .upload_base64_picture(&req, &arg.0)
        .await;
...
pub async fn page_picture(req: HttpRequest, arg: web::Query<PicturesPageDTO>) -> impl Responder {
    ...
    let vo = CONTEXT
        .oss_service
        .pictures_page(&req, &arg.into_inner())
        .await;
    return RespVO::from_result(&vo).resp_json();
}
...
pub async fn page_files(req: HttpRequest, arg: web::Query<FilesPageDTO>) -> impl Responder {
    ...
    let vo = CONTEXT
        .oss_service
        .files_page(&req, None, &arg.into_inner())
        .await;
    return RespVO::from_result(&vo).resp_json();
}
...
pub async fn files_delete(req: HttpRequest, arg: web::Query<FilesDTO>) -> impl Responder {
    ...
    let vo = CONTEXT
        .oss_service
        .files_delete(&req, &arg.into_inner())
        .await;
    return RespVO::from_result(&vo).resp_json();
}
...
/// 获取文件分页列表[公众]
#[get("/page/files/{id}")]
pub async fn public_page_files(
    req: HttpRequest,
    path: web::Path<u64>,
    arg: web::Query<FilesPageDTO>,
) -> impl Responder {
    log::info!("page_files:{:?}", arg.clone().into_inner());
    ...
    let vo = CONTEXT
        .oss_service
        .files_page(&req, Some(organize), &param)
        .await;
    return RespVO::from_result(&vo).resp_json();
}
