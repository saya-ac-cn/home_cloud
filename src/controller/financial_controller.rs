...
pub async fn page_journal(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    ...
    let vo = CONTEXT.financial_service.journal_page(&req,&arg.into_inner()).await;
    return RespVO::from_result(&vo).resp_json();
}
...
pub async fn excel_journal(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    ...
    let result = CONTEXT.financial_service.journal_excel(&req,&arg.into_inner()).await;
    return result;
}
...
/// 获取流水列表
#[get("/general/journal")]
pub async fn detail_general_journal(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    log::info!("detail_general_journal:{:?}", arg.clone().into_inner());
    let vo = CONTEXT.financial_service.general_journal_detail(&req,&arg.into_inner()).await;
    return RespVO::from_result(&vo).resp_json();
}

/// 导出流水明细报表
#[get("/general/journal/excel")]
pub async fn excel_general_journal(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    log::info!("excel_general_journal:{:?}", arg.clone().into_inner());
    let result = CONTEXT.financial_service.general_journal_excel(&req,&arg.into_inner()).await;
    return result;
}
...
/// 分页按天流水汇总数据
#[get("/journal/day")]
pub async fn page_journal_collect(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    log::info!("page_journal_collect:{:?}", arg.clone().into_inner());
    let vo = CONTEXT.financial_service.journal_collect_page(&req,&arg.into_inner()).await;
    return RespVO::from_result(&vo).resp_json();
}

/// 导出流水明细汇总
#[get("/journal/collect/excel")]
pub async fn excel_journal_collect(req: HttpRequest,arg: web::Query<JournalPageDTO>) -> impl Responder {
    log::info!("excel_journal_collect:{:?}", arg.clone().into_inner());
    let result = CONTEXT.financial_service.journal_collect_excel(&req,&arg.into_inner()).await;
    return result;
}
...
pub async fn order_month_journal(req: HttpRequest,arg: web::Query<JournalTotalDTO>) -> impl Responder {
...
pub async fn compute_pre6_journal(req: HttpRequest,arg: web::Query<JournalTotalDTO>) -> impl Responder {
