...
impl FinancialService {
    ...
    pub async fn delete_journal(&self, req: &HttpRequest,id: &u64) -> Result<u64> {
        ...
        let write_result = Journal::delete_by_id_organize(financial_rbatis_pool!(),&id,&user_info.organize).await;
    ...
    pub async fn delete_general_journal(&self, req: &HttpRequest,id: &u64) -> Result<u64>{
        ...
        let general_journal_count = GeneralJournalMapper::select_count_by_journal_id(financial_rbatis_pool!(),&journal_exist.id.clone().unwrap()).await?.unwrap();
        if general_journal_count > 1  {
            ...
            let write_result = Journal::delete_by_id_organize(financial_rbatis_pool!(),&general_journal_exist.journal_id.clone().unwrap(),&user_info.organize).await;
            if write_result.is_err(){
                error!("删除流水时，发生异常:{}",write_result.unwrap_err());
                return Err(Error::from("删除流水失败!"));
            }
    ...
    pub async fn journal_page(&self, req: &HttpRequest, param: &JournalPageDTO) -> Result<Page<JournalVO>>  {
        ...
        let count_result = JournalMapper::select_count(financial_rbatis_pool!(), &arg,&extend).await;
    ...
    pub async fn general_journal_detail(&self, req: &HttpRequest, param: &JournalPageDTO)-> Result<Vec<GeneralJournalVO>>  {
        ...
        let wrap_result = GeneralJournalMapper::select_detail(financial_rbatis_pool!(), &arg,&extend).await;
    ...
    pub async fn general_journal_excel(&self, req: &HttpRequest, param: &JournalPageDTO) -> HttpResponse {
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(total_row);
        let journal_result = JournalMapper::select_page(financial_rbatis_pool!(), &arg,&extend).await;
        ...
        // 联动查询明细
        let general_journal_result = GeneralJournalMapper::select_detail(financial_rbatis_pool!(), &arg,&extend).await;
    ...
    pub async fn journal_collect_page(&self, req: &HttpRequest, param: &JournalPageDTO) -> Result<Page<JournalVO>>  {
        ...
        let count_result = JournalMapper::select_day_count(financial_rbatis_pool!(), &arg,&extend).await;
        if count_result.is_err(){
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(result.page_size);
        let page_result = JournalMapper::select_day_page(financial_rbatis_pool!(), &arg,&extend).await;
        if page_result.is_err() {
    ...
    pub async fn journal_collect_excel(&self, req: &HttpRequest, param: &JournalPageDTO) -> HttpResponse {
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(total_row);
        let journal_result = JournalMapper::select_day_page(financial_rbatis_pool!(), &arg,&extend).await;
        if journal_result.is_err() {
        ...
        let journal_rows = journal_result.unwrap().unwrap();
        // 联动查询明细
        let general_journal_result = GeneralJournalMapper::select_day_collect(financial_rbatis_pool!(), &arg,&extend).await;
        if general_journal_result.is_err(){
    ...
    pub async fn compute_account_growth_rate(&self, req: &HttpRequest,month:&String) ->Result<HashMap<&str,Decimal>> {
        ...
        // 上月的统计
        let _last_month_wrap = JournalMapper::total_balance(financial_rbatis_pool!(), &user_info.organize,&last_month).await;
        ...
        // 去年同期
        let _last_year_wrap = JournalMapper::total_balance(financial_rbatis_pool!(), &user_info.organize,&last_year).await;
