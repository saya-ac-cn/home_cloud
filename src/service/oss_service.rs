...
impl OssService {
    ...
    pub async fn files_update(&self,req: &HttpRequest,arg:&FilesDTO) ->Result<u64>{
        ...
        LogMapper::record_log_by_jwt(primary_rbatis_pool!(),&user_info,String::from("OX015")).await;
    ...
    pub async fn files_delete(&self, req: &HttpRequest,arg:&FilesDTO) -> Result<u64>{
        ...
        let write_result = Files::delete_by_column(business_rbatis_pool!(),Files::id(), &files.id).await;
        if write_result.is_err(){
            error!("删除文件时，发生异常:{}",write_result.unwrap_err());
            return Err(Error::from("删除文件失败!"));
        }

        LogMapper::record_log_by_jwt(primary_rbatis_pool!(),&user_info,String::from("OX014")).await;
    ...
    pub async fn picture_delete(&self, req: &HttpRequest,id:u64) -> Result<u64>{
        ...
        let write_result = Pictures::delete_by_column(business_rbatis_pool!(),Pictures::id(), &id).await;
        if write_result.is_err(){
            error!("删除图片或壁纸时，发生异常:{}",write_result.unwrap_err());
            return Err(Error::from("删除图片或壁纸失败!"));
        }

        LogMapper::record_log_by_jwt(primary_rbatis_pool!(),&user_info,String::from("OX012")).await;
        return Ok(write_result?.rows_affected);
    }
    ...
    pub async fn upload_logo(&self,req: &HttpRequest,arg:&Base64PictureDTO) -> Result<String> {
        ...
        let write_result = User::update_by_column(primary_rbatis_pool!(),&user_exist,User::account()).await;
        if  write_result.is_err(){
            error!("在修改用户头像时，发生异常:{}",write_result.unwrap_err());
            return Err(Error::from("修改用户头像失败!"));
        }
        LogMapper::record_log_by_jwt(primary_rbatis_pool!(),&user_info,String::from("OX005")).await;
    ...
    pub async fn upload_file(&self, req: &HttpRequest,mut payload: Multipart) -> Result<String> {
        ...
        LogMapper::record_log_by_jwt(primary_rbatis_pool!(),&user_info,String::from("OX013")).await;
