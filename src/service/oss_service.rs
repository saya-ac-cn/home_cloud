...
use crate::domain::mapper::files_mapper::FilesMapper;
use crate::domain::mapper::log_mapper::LogMapper;
use crate::domain::mapper::pictures_mapper::PicturesMapper;
use crate::domain::table::{Files, Pictures,User};
...
impl OssService {
    ...
    pub async fn files_download(&self,id:u64) -> HttpResponse {
        ...
        let content_disposition = format!("attachment;filename*=utf-8''{}",&files.file_name.unwrap());
    ...
    pub async fn files_update(&self,req: &HttpRequest,arg:&FilesDTO) ->Result<u64>{
        ...
        let query_file_wrap = Files::select_by_id_and_organize(business_rbatis_pool!(),  &arg.id.clone().unwrap(),&user_info.organize).await;
        ...
        let files_exist = files_option.ok_or_else(|| Error::from((format!("id={} 的文件不存在!", &arg.id.clone().unwrap()),util::NOT_EXIST_CODE)))?;
        let files = Files{
            id:arg.id,
            uid: None,
            file_name: None,
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(),&user_info,String::from("OX015")).await;
    ...
    pub async fn files_delete(&self, req: &HttpRequest,arg:&FilesDTO) -> Result<u64>{
        ...
        let write_result = Files::delete_by_id(business_rbatis_pool!(),&files.id.clone().unwrap()).await;
        if write_result.is_err(){
            error!("删除文件时，发生异常:{}",write_result.unwrap_err());
            return Err(Error::from("删除文件失败!"));
        }

        LogMapper::record_log_by_context(primary_rbatis_pool!(),&user_info,String::from("OX014")).await;
    ...
    pub async fn picture_delete(&self, req: &HttpRequest,id:u64) -> Result<u64>{
        ...
        let picture = picture_op.ok_or_else(|| Error::from((format!("id为:{} 的图片或壁纸不存在!", id),util::NOT_EXIST_CODE)))?;
        // 判断文件是否存在，存在才删除
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(),&user_info,String::from("OX012")).await;
        return Ok(write_result?.rows_affected);
    }
    ...
    pub async fn pictures_page(&self, req: &HttpRequest, param: &PicturesPageDTO) -> Result<Page<PicturesVO>>  {
        ...
        let mut arg= param.clone();
        arg.organize = Some(user_info.organize);
        let count_result = PicturesMapper::select_count(business_rbatis_pool!(), &arg,&extend).await;
        if count_result.is_err(){
    ...
    pub async fn upload_logo(&self,req: &HttpRequest,arg:&Base64PictureDTO) -> Result<String> {
        ...
        let mut user_exist = user_option.ok_or_else(|| Error::from((format!("用户:{} 不存在!", &user_info.account.clone()),util::NOT_EXIST_CODE)))?;
        ...
        user_exist.update_time = DateTimeUtil::naive_date_time_to_str(&Some(DateUtils::now()),&util::FORMAT_Y_M_D_H_M_S);
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(),&user_info,String::from("OX005")).await;
    ...
    pub async fn upload_file_picture(&self,req: &HttpRequest,mut payload: Multipart) -> Result<String>{
        ...
        let user_option = query_user_wrap.unwrap().into_iter().next();
        user_option.ok_or_else(|| Error::from((format!("用户:{} 不存在!", &user_info.account.clone()),util::NOT_EXIST_CODE)))?;
        let today_op = DateTimeUtil::naive_date_time_to_str(&Some(DateUtils::now()), util::FORMAT_YMD);
        let today = today_op.unwrap();
    ...
    pub async fn upload_file(&self, req: &HttpRequest,mut payload: Multipart) -> Result<String> {
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(),&user_info,String::from("OX013")).await;
    ...
    pub async fn mkdir_if_not_exists(&self,save_path:&Path) -> Result<bool>{
        if !save_path.exists(){
            ...
            if create_result.is_err() {
                ...
                return Err(Error::from(("create folder fail",util::FILE_IO_ERROR_CODE)));
            }
        }
        Ok(true)
    }
