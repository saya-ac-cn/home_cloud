...
use crate::util::Page;
use crate::domain::table::{Pictures, Plan, PlanArchive, User};
...
impl SystemService {
    ...
    pub async fn login(&self, req: &HttpRequest, arg: &SignInDTO) -> Result<SignInVO> {
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(), &context, String::from("OX001")).await?;
    ...
    pub async fn generate_token(&self, req: &HttpRequest, user: &User,platform:&str) -> Result<SignInVO> {
        ...
        let query_picture_wrap = Pictures::select_by_column(business_rbatis_pool!(),"id",&user_vo.background).await;
        ...
        let today_plan_result_warp = primary_rbatis_pool!().query_decode::<Vec<HashMap<String, String>>>(query_today_plan_sql, vec![to_value!(user.account.clone()),to_value!(user.account.clone())]).await;
    ...
    pub async fn logout(&self, req: &HttpRequest) -> Result<UserContext> {
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(), &user_info.clone().unwrap(), String::from("OX002")).await?;
        Ok(user_info.unwrap())
    }
    ...
    pub async fn user_edit(&self, req: &HttpRequest, arg: &UserDTO) -> Result<u64> {
        ...
        LogMapper::record_log_by_context(primary_rbatis_pool!(), &context, String::from("OX003")).await?;
    ...
    pub async fn user_get_own_organize(&self, req: &HttpRequest) -> Result<Vec<UserOwnOrganizeVO>> {
        let context = UserContext::extract_user_by_request(req).await.ok_or_else(|| Error::from(util::NOT_AUTHORIZE_CODE))?;
        let query_result = UserMapper::select_own_organize_user(primary_rbatis_pool!(), &context.account).await;
    ...
    pub async fn user_update_password(&self, req: &HttpRequest, arg: &UserDTO) -> Result<u64> {
        ...
        let context = UserContext::extract_user_by_request(req).await.ok_or_else(|| Error::from(util::NOT_AUTHORIZE_CODE))?;
        LogMapper::record_log_by_context(primary_rbatis_pool!(), &context, String::from("OX004")).await?;
        Ok(result.unwrap().rows_affected)
    }
    ...
    pub async fn compute_pre6_logs(&self, req: &HttpRequest,month:&String) ->Result<Value> {
        ...
        let current_month_log_result_warp = primary_rbatis_pool!().query_decode::<u64>(query_current_month_log_sql, vec![to_value!(user_info.organize),to_value!(month.as_str())]).await;
        ...
        let compute_result_warp = primary_rbatis_pool!().query_decode::<Vec<TotalPre6MonthVO>>(query_sql.as_str(), vec![]).await;
    ...
    pub async fn compute_object_rows(&self, req: &HttpRequest)->Result<Value> {
        ...
        let query_notebook_result_warp = business_rbatis_pool!().query_decode(query_notebook_sql, vec![to_value!(user_info.organize)]).await;
        ...
        let primary_count_result_warp = primary_rbatis_pool!().query_decode(query_primary_count_sql.as_str(), vec![]).await;
        ...
        let business_count_result_warp = business_rbatis_pool!().query_decode::<Vec<TotalTable>>(query_business_count_sql.as_str(), vec![]).await;
    ...
    pub async fn edit_plan_archive(&self, req: &HttpRequest,arg: &PlanArchiveDTO) -> Result<u64> {
        ...
        let query_plan_archive_wrap = PlanArchive::select_by_id(primary_rbatis_pool!(), &arg.id.unwrap()).await;
    ...
    pub async fn plan_grid(&self,organize:&u64,query_month:&str) -> Result<serde_json::Value>{
        ...
        let compute_result_warp = primary_rbatis_pool!().query_decode::<Vec<PlanArchiveVO>>(query_sql.as_str(), vec![]).await;
        if compute_result_warp.is_err(){
